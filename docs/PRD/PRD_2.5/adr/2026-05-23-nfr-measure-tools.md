...
| 항목 | 측정 도구 | 임계값 | 측정 방식 |
|---|---|---|---|
| 반응속도(카드 → DOM) | `tests/e2e/preview-latency.spec.ts` (Playwright + Chromium) | 중앙값 < 100ms | localStorage seed로 25개 카드 활성 워크스페이스를 직접 진입, `MutationObserver`로 `WORK_PREVIEW_CONTENT` 갱신 시점 포착, `performance.now()` 차이 30회 측정 |
...
5. **25개 카드 시나리오**: 단일 트리 정의(예: `feature-impl` 13개)로는 PRD 명시 25개 활성 상태를 재현할 수 없어, E2E 측정에서는 `card-definitions.json` 27개 중 25개를 임의로 `active: true`로 구성한 localStorage 세션을 주입하여 워크스페이스에 직접 진입한다. 실제 운영 시나리오에서 25개를 동시 활성화하는 경우는 드물지만, 본 측정은 NFR의 stress 한계를 검증하는 목적이다.
