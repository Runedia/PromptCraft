...
const { generate, preview, CONTEXT_FORMATS } = require('../../core/context/index');
const {
  askText,
  askMultiline,
  askConfirm,
...
const { ScanError, ContextError } = require('../../shared/errors');
const { resolvePath } = require('../../shared/utils');
...
const cmd = new Command('context')
  ...
  .action(async (pathArg, options) => {
    try {
      ...
      const formats = options.format.split(',').map(f => f.trim()).filter(Boolean);
      for (const fmt of formats) {
        if (!CONTEXT_FORMATS[fmt]) {
          await error(`지원하지 않는 포맷입니다: ${fmt}\n사용 가능: ${Object.keys(CONTEXT_FORMATS).join(', ')}`);
          process.exit(1);
        }
      }
