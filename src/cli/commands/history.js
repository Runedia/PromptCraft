...
const { success, error, info, warn, section, getChalk, askConfirm } = require('../ui/prompts');
const { formatDate, truncate } = require('../../shared/utils');
...
async function printList(rows) {
  ...
  console.log(
    chalk.bold(headerId) +
    chalk.bold(headerDate) +
    chalk.bold(headerSit) +
    chalk.bold(headerPrev)
  );
