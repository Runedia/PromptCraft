...
function ResultScreen({ wizard, options, inkComponents }) {
  ...
  React.useEffect(() => {
    ...
    import('clipboardy').then(mod => mod.default.write(prompt))
      .then(() => {
  ...
  const bannerEl = statusMsg
    ? React.createElement(Box, { marginBottom: 1, paddingX: 1 },
        React.createElement(Text, { color: copyStatus === 'error' ? 'red' : 'green' }, `  ${statusMsg}`)
      )
    : null;

  // 프롬프트 텍스트 영역
  const contentEl = React.createElement(Box, { flexDirection: 'column', marginBottom: 1 },
    ...visibleLines.map((line, i) =>
      React.createElement(Box, { key: `line-${scrollOffset + i}` },
        React.createElement(Text, null, `  ${line}`)
      )
    ),
    React.createElement(Box, { justifyContent: 'flex-end', paddingRight: 1 },
      React.createElement(Text, { dimColor: true }, `[${currentPage}/${totalPages} 페이지]`)
    )
  );

  // 액션 바 (view 단계)
  const actionBar = React.createElement(Box, {
    borderStyle: 'single',
    borderColor: 'gray',
    paddingX: 1,
  },
    ...
    return React.createElement(Box, {
      ...
      React.createElement(Box, { marginLeft: 2 },
        React.createElement(TextInput, {
          ...
          onSubmit: (name) => {
            try {
              db.template.save({ name: name.trim(), treeId: wizard.treeId, answers: wizard.answers });
  ...
  return React.createElement(Box, {
    ...
    contentEl,
    actionBar,
    React.createElement(Box, { marginTop: 0, paddingX: 2 },
