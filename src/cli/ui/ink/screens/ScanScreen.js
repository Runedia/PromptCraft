...
const ScanSummary = require('../components/ScanSummary');
const { scan }    = require('../../../../core/scanner/index');
const { LAST_SCAN_PATH } = require('../../../../shared/constants');
...
function ScanScreen({ wizard, options, inkComponents }) {
  ...
  const cached = React.useMemo(() => {
    ...
    try { return JSON.parse(fs.readFileSync(LAST_SCAN_PATH, 'utf8')); } catch { return null; }
  }, []);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    scan(scanPath).then(result => {
      if (!cancelled) {
        // 캐시 저장
        try { fs.writeFileSync(LAST_SCAN_PATH, JSON.stringify(result), 'utf8'); } catch (_) {}
  ...
  if (phase === 'select') {
    ...
    body = React.createElement(Box, { marginLeft: 2 },
