...
const { autocompleteFilePathSync } = require('../../file-mention');

function useFileComplete(projectRoot) {
  const [candidates, setCandidates] = React.useState([]);
  const [dropdownIdx, setDropdownIdx] = React.useState(0);

  const updateQuery = React.useCallback((partial) => {
  ...
  const moveDown = React.useCallback(() => setDropdownIdx(i =>
    candidates.length > 0 ? Math.min(candidates.length - 1, i + 1) : 0
  ), [candidates.length]);
...
module.exports = { useFileComplete };
