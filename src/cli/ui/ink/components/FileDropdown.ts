...
function FileDropdown({ candidates, activeIdx, inkComponents }) {
  ...
  const windowStart = Math.max(
    0,
    Math.min(activeIdx - Math.floor(PAGE / 2), candidates.length - PAGE)
  );
  const visible = candidates.slice(windowStart, windowStart + PAGE);

  const hasMore = windowStart + PAGE < candidates.length;
  const hasAbove = windowStart > 0;
  const header = hasAbove ? `┌── 파일 목록 (↑ 더 있음) ──` : '┌── 파일 목록 ──────────────';
  const footer = hasMore ? '└── ↓ 더 있음 ──────────────' : '└───────────────────────────';
  ...
  return React.createElement(
    ...
    React.createElement(Box, null, React.createElement(Text, { dimColor: true }, header)),
    ...visible.map((c, i) => {
    ...
    React.createElement(Box, null, React.createElement(Text, { dimColor: true }, footer))
  );
}
