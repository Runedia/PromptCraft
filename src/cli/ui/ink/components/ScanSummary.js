...
function ScanSummary({ scanResult, inkComponents }) {
  ...
  const langElements = displayLangs.map((lang, i) => {
    ...
    return React.createElement(Box, { key: `lang-${i}`, marginLeft: 2 },
  ...
  if (hiddenCount > 0) {
    langElements.push(
      React.createElement(Box, { key: 'lang-more', marginLeft: 2 },
  ...
  if (packageManager) {
    badges.push(
      React.createElement(Text, { key: 'pm', color: 'yellow' }, `${packageManager}  `)
    );
  }
