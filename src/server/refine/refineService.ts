...
import type { RefineAssessment, RefineConfig, RefineMode } from '../../core/refine/types.js';
...
/** 메시지 조립 → LLM 호출 → 파싱. createClient 주입으로 테스트 가능. */
export async function assessPrompt(opts: AssessOpts): Promise<RefineAssessment> {
  ...
  const messages = buildRefineMessages(opts.promptText, opts.lang, opts.mode);
